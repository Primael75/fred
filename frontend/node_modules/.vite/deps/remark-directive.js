import {
  parseEntities
} from "./chunk-6TGRTLDZ.js";
import {
  ccount
} from "./chunk-FVNALRYS.js";
import {
  factoryWhitespace
} from "./chunk-VFM5BBFG.js";
import {
  codes,
  constants,
  factorySpace,
...
function directiveToMarkdown(options) {
  ...
  function attributes4(node, state) {
    const attributes5 = node.attributes || {};
    const values = [];
    let classesFull;
    let classes;
    let id;
    ...
    for (key2 in attributes5) {
      if (own2.call(attributes5, key2) && attributes5[key2] !== void 0 && attributes5[key2] !== null) {
        const value = String(attributes5[key2]);
        if (key2 === "id") {
          id = settings.preferShortcut !== false && shortcut.test(value) ? "#" + value : quoted("id", value, node, state);
        } else if (key2 === "class") {
          ...
          while (++index < list.length) {
            ;
            (settings.preferShortcut !== false && shortcut.test(list[index]) ? classesList : classesFullList).push(list[index]);
          }
          classesFull = classesFullList.length > 0 ? quoted("class", classesFullList.join(" "), node, state) : "";
          classes = classesList.length > 0 ? "." + classesList.join(".") : "";
        } else {
          values.push(quoted(key2, value, node, state));
        }
      }
    }
    if (classesFull) {
  ...
  function quoted(key2, value, node, state) {
    if (settings.collapseEmptyAttributes !== false && !value) return key2;
    ...
    const appliedQuote = settings.quoteSmart && ccount(value, preferred) > ccount(value, alternative) ? alternative : preferred;
    const subset = node.type === "textDirective" ? [appliedQuote] : [appliedQuote, "\n", "\r"];
    return key2 + "=" + appliedQuote + stringifyEntitiesLight(value, { subset }) + appliedQuote;
  }
}
...
function factoryAttributes(effects, ok2, nok, attributesType, attributesMarkerType, attributeType, attributeIdType, attributeClassType, attributeNameType, attributeInitializerType, attributeValueLiteralType, attributeValueType, attributeValueMarker, attributeValueData, disallowEol) {
  ...
  function between(code) {
    ...
    if (code === codes.eof || markdownLineEnding(code) || unicodeWhitespace(code) || unicodePunctuation(code) && code !== codes.dash && code !== codes.underscore) {
      return end(code);
    }
  ...
  function name(code) {
    if (code === codes.eof || markdownLineEnding(code) || unicodeWhitespace(code) || unicodePunctuation(code) && code !== codes.dash && code !== codes.dot && code !== codes.colon && code !== codes.underscore) {
...
function factoryName(effects, ok2, nok, type) {
  ...
  return start;
  function start(code) {
    if (code === codes.eof || markdownLineEnding(code) || unicodePunctuation(code) || unicodeWhitespace(code)) {
      return nok(code);
    }
    effects.enter(type);
    effects.consume(code);
  ...
  function name(code) {
    if (code === codes.eof || markdownLineEnding(code) || unicodeWhitespace(code) || unicodePunctuation(code) && code !== codes.dash && code !== codes.underscore) {
      effects.exit(type);
      return self.previous === codes.dash || self.previous === codes.underscore ? nok(code) : ok2(code);
    }
    effects.consume(code);
    return name;
  }
...
function tokenizeDirectiveContainer(effects, ok2, nok) {
  ...
  function afterLabel(code) {
    return code === codes.leftCurlyBrace ? effects.attempt(attributes, afterAttributes, afterAttributes)(code) : afterAttributes(code);
  }
